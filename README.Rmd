...
```{r eval=FALSE}
# we group cascades by the users who posted the tweet
names(cascades) <- sapply(cascades, function(data) data$user[[1]])
...
```
In `evently`, we group cascades posted by the same Twitter user together. We assume models built on the groups of cascades characterize the temporal dynamics of a user's influence to others.

## User influence analysis
We use the parameters of `group_fits` from `evently` to quantify a user.
